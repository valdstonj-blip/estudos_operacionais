...
import { Inscricao, FilterState } from '../types';
import { Phone, Mail, User, Eye, Copy, Check, MessageSquare, ArrowUp, ArrowDown, ArrowUpDown } from 'lucide-react';
...
interface InscricoesTableProps {
  ...
  onHeaderSort?: (column: FilterState['sortBy']) => void;
}
...
export const InscricoesTable: React.FC<InscricoesTableProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm shadow-slate-200/50 overflow-hidden">
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead className="sticky top-0 z-20">
            <tr className="bg-slate-900 text-white font-semibold uppercase tracking-wider text-[11px] border-b border-slate-800 shadow-sm">
              <th
                onClick={() => onHeaderSort?.('timestamp')}
                className={`py-3 px-3.5 whitespace-nowrap bg-slate-900 ${
                  onHeaderSort ? 'cursor-pointer hover:bg-slate-800 select-none' : ''
                }`}
                title="Clique para alternar ordenação por data/hora de envio (carimbo da planilha)"
              >
                <div className="flex items-center gap-1">
                  <span># Envio / Carimbo</span>
                  {renderSortIcon('timestamp')}
                </div>
              </th>
              <th
                onClick={() => onHeaderSort?.('data')}
                className={`py-3 px-3.5 whitespace-nowrap bg-slate-900 ${
                  onHeaderSort ? 'cursor-pointer hover:bg-slate-800 select-none' : ''
                }`}
                title="Clique para ordenar por data da capacitação"
              >
                <div className="flex items-center gap-1">
                  <span>Data Capacitação</span>
                  {renderSortIcon('data')}
                </div>
              </th>
              <th
                onClick={() => onHeaderSort?.('opm')}
                className={`py-3 px-3.5 whitespace-nowrap bg-slate-900 ${
                  onHeaderSort ? 'cursor-pointer hover:bg-slate-800 select-none' : ''
                }`}
                title="Clique para ordenar por OPM"
              >
                <div className="flex items-center gap-1">
                  <span>CPA / OPM</span>
                  {renderSortIcon('opm')}
          ...
          <tbody className="divide-y divide-slate-100 text-slate-700">
            {inscricoes.map((item, index) => {
              ...
              return (
                ...
                >
                  {/* Carimbo de Envio & Ordem */}
                  <td className="py-3 px-3.5 align-top whitespace-nowrap">
                    <div className="flex items-center gap-1.5">
                      <span className="w-5 h-5 rounded-full bg-slate-100 text-slate-600 font-bold text-[10px] flex items-center justify-center border border-slate-200">
                        {index + 1}
                      </span>
                      <div className="flex flex-col">
                        <span className="font-mono text-slate-800 font-semibold text-[11px]">
                          {item.timestamp || 'Sem carimbo'}
                        </span>
                      </div>
                    </div>
                  </td>
      ...
      {/* Footer bar showing count & active sorting */}
      <div className="px-4 py-2 bg-slate-50 border-t border-slate-200 text-slate-500 text-xs flex justify-between items-center">
        <span>
          Total de registros nesta visão: <strong className="text-slate-800">{inscricoes.length}</strong>
        </span>
        <span className="text-[11px] text-slate-400">
          Ordenação: {sortBy === 'timestamp' ? 'Carimbo de Envio (Ordem da Planilha)' : sortBy === 'data' ? 'Data da Capacitação' : sortBy === 'opm' ? 'OPM / Unidade' : sortBy === 'cpa' ? 'Comando CPA' : 'Qtd. Efetivo'} ({sortOrder === 'asc' ? 'Crescente' : 'Decrescente'})
        </span>
      </div>
    </div>
  );
